      ...
      {open && (
        <div className="absolute z-10 mt-1 max-h-48 w-full overflow-y-auto rounded-control border border-border bg-surface shadow-lg">
          <button
            ...
            className="block w-full px-3 py-2 text-right text-sm font-medium text-brand-accent hover:bg-surface-muted"
          >
          ...
          {filtered.map((s) => (
            <button
              ...
              className="block w-full truncate px-3 py-2 text-right text-sm text-text-primary hover:bg-surface-muted"
            >
              {s.display_name}
            </button>
          ))}
